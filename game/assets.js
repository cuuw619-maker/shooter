...
export function buildMapColliders(root, limit = 320) {
  const candidates = [];
  root.updateMatrixWorld(true);
  ...
  candidates.sort((a,b) => (b.halfX * b.halfZ) - (a.halfX * a.halfZ));

  const colliders = [];
  for (const candidate of candidates) {
    // Drop almost-identical nested boxes generated by skinned/combined GLB parts.
    const duplicate = colliders.some(existing =>
      Math.abs(existing.x - candidate.x) < 0.12 &&
      Math.abs(existing.z - candidate.z) < 0.12 &&
      Math.abs(existing.halfX - candidate.halfX) < 0.12 &&
      Math.abs(existing.halfZ - candidate.halfZ) < 0.12
    );
    if (!duplicate) colliders.push(candidate);
